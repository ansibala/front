...
const TeamMemberForm = (props) => {
  ...
  const dialogData = JSON.parse(localStorage.getItem("updateTeamMemberDataDialogue"));
  ...
  useEffect(() => {
    ...
    if (tmdbId?.IMDBid == null && tmdbId?.TmdbMovieId == null) {
      const fetchData = async () => {
        const fileNameWithExtensionThumbnail = dialogData?.image
          ?.split("/")
          .pop();
        const { imageURL } = await covertURl(
          "RoleImage/" + fileNameWithExtensionThumbnail
        );
  ...
  let folderStructureTrailerImage = "mova/RoleImage";
